...
alias makec='function _makec() { gcc -Wall -Wextra -std=c11 -O2 "$1" -o "${1%.*}" && echo "Compiled successfully to ${1%.*}"; }; _makec'
alias makecpp='function _makecpp() { g++ -Wall -Wextra -std=c++17 -O2 "$1" -o "${1%.*}" && echo "Compiled successfully to ${1%.*}"; }; _makecpp'
...
__conda_setup="$('/home/habert/anaconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
